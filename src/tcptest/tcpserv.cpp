...
// Constants
// --------------------------------------------------------------
const char *version_str = "2.03";
const char *program_name = "tcpserv";
...
void HelpMessage() 
{
  cout << "TCP bandwidth benchmark server version " << version_str << "\n" 
       << flush;
...
int ProcessArgs(char *arg)
{
  ...
  switch(arg[1]) {
    case 'c': case 'C':
      servercfg->display_processor_clock = 1;
      break;
...
int StartServer()
{
  ...
  cout << "TCP LAN/WAN benchmark server" << "\n" 
       << flush;
